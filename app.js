 ...
 **/
let worker
let rooms = {}          // { roomId1: { Router, rooms: [ socketId1, ... ] }, ...}
...
connections.on('connection', async socket => {
  ...
  // see client's socket.emit('producerTransportConnect', ...)
  socket.on('producerTransportConnect', ({ dtlsParameters }) => {
    console.log('DTLS PARAMS... ', { dtlsParameters })
    
    getTransport(socket.id).connect({ dtlsParameters })
  })

  // see client's socket.emit('transportProduce', ...)
  ...
  // see client's socket.emit('recvTransportConnect', ...)
  socket.on('recvTransportConnect', async ({ dtlsParameters, serverConsumerTransportId }) => {
    console.log(`DTLS PARAMS: ${dtlsParameters}`)
